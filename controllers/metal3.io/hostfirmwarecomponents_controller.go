...
func (r *HostFirmwareComponentsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	info.log.Info("updateHostFirmware function called")
	...
	info.log.Info("updateHostFirmware exited")
...
func (r *HostFirmwareComponentsReconciler) updateHostFirmware(info *rhfcInfo, components []metal3api.FirmwareComponentStatus) (err error) {
	...
	if updatesMismatch {
		info.log.Info("updateHostFirmware updatesMismatch True")
		if setUpdatesCondition(generation, &newStatus, info, metal3api.HostFirmwareComponentsChangeDetected, metav1.ConditionTrue, reason, "") {
			info.log.Info("updateHostFirmware updatesMismatch True - HostFirmwareComponentsChangeDetected Condition True")
			dirty = true
		}
		...
		if err != nil {
			...
			if setUpdatesCondition(generation, &newStatus, info, metal3api.HostFirmwareComponentsValid, metav1.ConditionFalse, reason, fmt.Sprintf("Invalid Firmware Components: %s", err)) {
				info.log.Info("updateHostFirmware updatesMismatch True + Validation Failed - HostFirmwareComponentsValid Condition False")
				dirty = true
			}
		} else if setUpdatesCondition(generation, &newStatus, info, metal3api.HostFirmwareComponentsValid, metav1.ConditionTrue, reason, "") {
			info.log.Info("updateHostFirmware updatesMismatch True + Validation Failed - HostFirmwareComponentsValid Condition True")
			dirty = true
		}
	} else {
		info.log.Info("updatesMismatch False")
		if setUpdatesCondition(generation, &newStatus, info, metal3api.HostFirmwareComponentsValid, metav1.ConditionTrue, reason, "") {
			info.log.Info("updateHostFirmware updatesMismatch False - HostFirmwareComponentsValid Condition True")
			dirty = true
		}
		if setUpdatesCondition(generation, &newStatus, info, metal3api.HostFirmwareComponentsChangeDetected, metav1.ConditionFalse, reason, "") {
			info.log.Info("updateHostFirmware updatesMismatch False - HostFirmwareComponentsChangeDetected Condition False")
			dirty = true
		}
	}
	...
	if dirty {
		info.log.Info("updateHostFirmware Status for HostFirmwareComponents changed ")
